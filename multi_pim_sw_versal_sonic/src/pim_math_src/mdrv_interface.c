...
#ifdef CLP
int CLinternalMemcpy(void *pim_dst, void *pim_src, int size, int fpga_id, uint32_t cl_size) {
// int CLinternalMemcpy(void *pim_dst, void *pim_src, int size, int fpga_id, cl_data_t cl_data) {
    if (pl_dma != NULL) {
        struct pl_dma_t *pl_dma_node;
        pl_dma_node = &pl_dma[fpga_id];
        pim_args copy_code;
        ...
        copy_code.dstC_va = (uint64_t)pim_dst;
        copy_code.srcA_size = size;
        copy_code.cl_size = cl_size;
        // copy_code.cl_data = cl_data;
        // pthread_mutex_lock(&pl_dma_node->lock);
		if (ioctl(pl_dma_node->fd, MEMCPY_PL2PL, &copy_code) < 0) {
            printf("\033[0;31m internal DMA transaction failed! \033[0m\n");
            // pthread_mutex_unlock(&pl_dma_node->lock);
...
#ifdef PMM
    void set_piu_base_addr(uint64_t pmm_base_addr) {
        if (pl_dma != NULL) {
            struct pl_dma_t *pl_dma_node;
            pl_dma_node = &pl_dma[0];
            pim_args pim_args;
            pim_args.pmm_base_addr = pmm_base_addr;
            // pthread_mutex_lock(&pl_dma_node->lock);
            if (ioctl(pl_dma_node->fd, PIU_BASE_ADDR_UPDATE, &pim_args) < 0) {
                printf("PMM SET FAILED");
                // pthread_mutex_unlock(&pl_dma_node->lock);
                return -1;
            }
            // pthread_mutex_unlock(&pl_dma_node->lock);
