...
long pl_dma_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    clp_clr_signal = (void *)cdma_dev->config_reg + CONF_OFFSET_CLP_CLR;
#ifdef PMM
    uint32_t *piu_base_reg; //  pdmm
    piu_base_reg = (void *)cdma_dev->config_reg + CONF_OFFSET_PIU_BASE_REG; //  pdmm
#endif
    ...
    switch (cmd)
    {
        ...
        {   
             ...
             */
            // printk("desc_idx: %d\n", pim_args.desc_idx);
            // printk("(pim_args.desc_idx + 1) * 0x40): %d\n", (pim_args.desc_idx + 1) * 0x40);
        #ifndef QDMA
            ...
            #endif
            // for (i = 0; i < AIMX_SPIM_CH_NUM; i++) {
            //     *((&clr_signal[0]) + (i * 0x20)/4) = 0; //0x5000
            //     *((&pim_signal[0]) + (i * 0x20)/4) = 0; //0x4000
            // }
            clr_signal[i] = 0;
            pim_signal[i] = 0;
            ...
            clp_clr_signal[0] = 0;
            // for (i = 0; i < AIMX_SPIM_CH_NUM; i++) {
            //         clr_signal[i] = 0; //0x4000
            //         pim_signal[i] = 0; //0x5000
            // }
            // for (i = 0; i < AIMX_SPIM_CH_NUM; i++) {  
            //     *((&clr_signal[0]) + (i * 0x20)/4) = 0; //0x5000
            //     *((&pim_signal[0]) + (i * 0x20)/4) = 0; //0x4000
        ...
        case MEMCPY_PL2PL: /* Memory copy using DMA engine */
        {
            ...
            tx_time          = 0;
            #endif
            // if (dma_single_tx(0x00800000, HIGH_ADDR, 0x6F00, HIGH_ADDR, 32, cdma_dev) < 0) {
            //     // mutex_unlock(&extio_mutex);
            //     return -EDMA_TX;
            // }
            #ifdef PERF_TIME
...
#ifdef PMM
        case PIU_BASE_ADDR_UPDATE:  //  pdmm
        {
            int k;
            for (k = 0; k < AIMX_SPIM_CH_NUM; k++){
                *(&piu_base_reg[0] + k * 0x20/4) = pim_args.pmm_base_addr;
                // piu_base_reg[4*k] = pim_args.pdmm_base_addr;
                // printk("piu_base_reg_addr CH %d: %llx\n", k, &piu_base_reg[0] + k * 0x20/4);
            }
            break;
        }
#endif       
