...
#define PRD_PWR	//	use of pread()/pwrite() is recommended over a sequence of lseek() + read()/write()
			...
			uint64_t base)
{
	...
	do { /* Support zero byte transfer */
		...
		rc = read(fd, buf, bytes);
			...
			// uint64_t dur_qdma = BILLION * (end_qdma.tv_sec - statrt_qdma.tv_sec) + (end_qdma.tv_nsec - statrt_qdma.tv_nsec);
