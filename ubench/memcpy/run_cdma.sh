...
TOTAL_AVERAGE_TIME="total_aveg_time.txt"
# Output the average time and transaction size
# echo "Total time: $TOTAL_TIME nanoseconds"  # Debugging statement
echo "Average CDMA Memcpy time over $X trials: $AVERAGE_TIME nanoseconds" >> $TOTAL_AVERAGE_TIME 
echo "Transaction size: $SIZE * 4B = $TRANSACTION_SIZE bytes" >> $TOTAL_AVERAGE_TIME
