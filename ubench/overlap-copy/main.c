...
static struct option const long_opts[] = {
	...
	{"bl_test", no_argument, NULL, 'b'},
	{"cl_size", no_argument, NULL, 'c'},
	{0, 0, 0, 0}
};
...
void* OverlapCopy(void* thread_args) {
	...
	uint32_t cl_size = args->cl_size;
	unsigned long largest_dur_qdma = 0;
	unsigned long smallest_dur_qdma = 999999999;
	unsigned long largest_dur_cdma = 0;
	unsigned long smallest_dur_cdma = 999999999;
	    struct timespec ts;
    ts.tv_sec = 0;
    ...
    switch (tid) {
        case 0: {
			...
			printf("QDMA Start time: %lu nanoseconds \n", BILLION * statrt_qdma.tv_sec + statrt_qdma.tv_nsec);
			clock_gettime(CLOCK_MONOTONIC, &statrt_qdma);	
			//   ret = write_from_buffer(args->devname, args->qdma_fd, args->Src, args->size, args->offset); //Read
			  ret = read_to_buffer(args->devname, args->qdma_fd, args->Dst, args->size, args->offset);        //
			  if(ret<0) return -1;
			clock_gettime(CLOCK_MONOTONIC, &end_qdma);
			printf("QDMA End time:   %lu nanoseconds \n", BILLION * end_qdma.tv_sec + end_qdma.tv_nsec);
			dur_qdma = BILLION * (end_qdma.tv_sec - statrt_qdma.tv_sec) + (end_qdma.tv_nsec - statrt_qdma.tv_nsec);
			if (smallest_dur_qdma > dur_qdma) smallest_dur_qdma = dur_qdma;
			if (largest_dur_qdma < dur_qdma) largest_dur_qdma = dur_qdma;
			// dur_qdma_total += dur_qdma;
			printf("QDMA Memcpy time: %lu nanoseconds \n", dur_qdma);			
			}
			printf("Smallest QDMA Memcpy time: %lu nanoseconds \n", smallest_dur_qdma);
			printf("Largest QDMA Memcpy time: %lu nanoseconds \n", largest_dur_qdma);
			break;
        ...
        case 1: {
			...
			printf("CDMA Start time: %lu nanoseconds \n", BILLION * statrt_cdma.tv_sec + statrt_cdma.tv_nsec);			
			clock_gettime(CLOCK_MONOTONIC, &statrt_cdma);	
              if(CLinternalMemcpy(args->Dst, args->Src, args->size/(cl_size+1), DEV0_CDMA, cl_size)<0){
				return -1;
			  }
			clock_gettime(CLOCK_MONOTONIC, &end_cdma);	
			printf("CDMA End time:   %lu nanoseconds \n", BILLION * end_cdma.tv_sec + end_cdma.tv_nsec);		  
			dur_cdma = BILLION * (end_cdma.tv_sec - statrt_cdma.tv_sec) + (end_cdma.tv_nsec - statrt_cdma.tv_nsec);

			// if (largest_dur_cdma < dur_cdma) largest_dur_cdma = dur_cdma;
			if (smallest_dur_cdma > dur_cdma) smallest_dur_cdma = dur_cdma;
			// dur_cdma_total += dur_cdma;
			printf("CDMA Memcpy time: %lu nanoseconds \n", dur_cdma); 
			}
			printf("Smallest CDMA Memcpy time: %lu nanoseconds \n", smallest_dur_cdma);
			// printf("Largest CDMA Memcpy time: %lu nanoseconds \n", largest_dur_cdma);
            break;
        }
        default:
...
int main(int argc, char *argv[]) {
	...
	while ((cmd_opt = getopt_long(argc, argv, "vhx:f:d:a:s:bc:", long_opts, NULL)) != -1) {
		switch (cmd_opt) {
		case 0:
			/* long option */
			break;
	...
	printf("Size: %lu(elemets), %lu(bytes)\n", size, size*sizeof(float));
    	...
    	for (int i = 0; i < 2; i++) {
			...
			if(i==0){
				...
				// thread_args[i].offset = 0x0100AA00;
